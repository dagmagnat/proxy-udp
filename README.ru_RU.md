...
`Proxy UDP` — интерактивный менеджер TCP/UDP-перенаправлений на базе Linux `iptables` NAT.

Несмотря на название, скрипт умеет работать не только с UDP. Он может создавать перенаправления для:
...
Параметр `?$(date +%s)` нужен, чтобы обойти возможный кеш GitHub Raw сразу после обновления файла.
...
В репозитории также есть `mtproto-manager` — интерактивный менеджер для запуска MTProto Proxy в Docker.
...
- Для MTProto Manager: Docker. Если Docker не установлен, менеджер попробует установить его на Debian/Ubuntu.
